                                ...
                                rpl_group_info *serial_rgi)
{
  ...
  */
  ...
  if (ev)
  {
    ...
    /*
      During provisioning, slave is expecting only row events with data or
      specially marked query events with meta-data
    */
    if (rli->mi->provisioning_mode && typ == QUERY_EVENT &&
        (ev->flags & LOG_EVENT_PROVISIONING_F) == 0)
    {
      Query_log_event *q_ev= (Query_log_event*)ev;

      /*
        Ignore COMMIT and ROLLBACK events which are generated by server itself
      */
      if (!q_ev->is_commit() && !q_ev->is_rollback())
      {
        rli->report(ERROR_LEVEL, ER_PROVISIONING_DDL_INTERRUPT, NULL,
                    ER(ER_PROVISIONING_DDL_INTERRUPT), q_ev->query);
...
static int queue_event(Master_info* mi,const char* buf, ulong event_len)
{
  ...
  */
  ...
  /*
    Provisioning events do not increase master_log_pos as they are not present
    in masters binlog, just generated by server and sent to slave
  */
  if (uint2korr(buf + FLAGS_OFFSET) & LOG_EVENT_PROVISIONING_F)
    inc_pos= 0;
